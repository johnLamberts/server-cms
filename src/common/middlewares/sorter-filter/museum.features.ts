...
export const museumFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    let query = supabase.from("museums").select("*", { count: 'exact' });
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('museums').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
    ...
    next();
      ...
      console.log(`[museumsErrorFeatures]: ${error}`)
      return next(error)
    }
  } 
}

export default museumFeature;
