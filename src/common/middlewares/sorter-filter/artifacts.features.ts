...
export const artifactsFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('artifacts').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
    ...
    next();
      ...
      console.log(`[artifactsErrorFeatures]: ${JSON.stringify(error, null, 4  )}`)
      return next(error)
    }
  } 
}

export default artifactsFeature;
