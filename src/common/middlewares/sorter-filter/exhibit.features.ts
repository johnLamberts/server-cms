...
export const exhibitsFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    let query = supabase.from("exhibits").select("*", { count: 'exact' });
    ...
    if (req.query.search) {
      ...
      query = query.or(
        `title.ilike.%${searchText}%,fee.ilike.%${searchText}%,address.ilike.%${searchText}%`
      )
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('exhibits').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
    ...
    next();
      ...
      console.log(`[exhibitsErrorFeatures]: ${error}`)
      return next(error)
    }
  } 
}

export default exhibitsFeature;
