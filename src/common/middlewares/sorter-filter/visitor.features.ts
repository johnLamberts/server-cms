...
export const visitorFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    let query = supabase.from("visitor").select("*", { count: 'exact' });
    ...
    if (req.query.search) {
      ...
      query = query.or(
        `firstName.ilike.%${searchText}%,lastName.ilike.%${searchText}%,middleName.ilike.%${searchText}%,email.ilike.%${searchText}%`
      )
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('visitor').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
    ...
    next();
      ...
      console.log(`[VisitorErrorFeatures]: ${error}`)
      return next(error)
    }
  } 
}

export default visitorFeature;
