...
export const eventFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    if (req.query.search) {
      ...
      query = query.or(
        `title.ilike.%${searchText}%,address.ilike.%${searchText}%`
      )
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('events').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
    ...
    next();
      ...
      console.log(`[eventsErrorFeatures]: ${error}`)
      return next(error)
    }
  } 
}
