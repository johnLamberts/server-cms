...
export const exhibitionFeature = () => {
  return async (req: TPaginationRequest, res: TPaginationResponse, next: NextFunction) => {
    ...
    let query = supabase.from("exhibition_type").select("*", { count: 'exact' });
    ...
    if (req.query.search) {
      ...
      query = query.or(
        `exhibition_type.ilike.%${searchText}%`
      )
    ...
    if(req.query.fields) {
      ...
      ({ data, count, error } = await supabase.from('exhibition_type').select(fields, { count: 'exact' }).range(offset, offset + limit - 1));
      if (error) {
        throw error;
      }    
    }
    ...
    next();
      ...
      console.log(`[ExhibitionErrorFeatures]: ${error}`)
      return next(error)
    }
  } 
}

export default exhibitionFeature;
